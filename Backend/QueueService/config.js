...
module.exports = {
  ...
  dmMinPlayers: Math.max(2, Math.min(4, toInt(process.env.DM_MIN_PLAYERS, 2))),
  dmMaxPlayers: Math.max(2, Math.min(4, toInt(process.env.DM_MAX_PLAYERS, 4))),
  dmQueueTimeoutMs: Math.max(2000, toInt(process.env.DM_QUEUE_TIMEOUT_MS, 8000)),
  dmMatchDurationMs: Math.max(60000, toInt(process.env.DM_MATCH_DURATION_MS, 300000)),
  ...
  weaponPickTimeoutMs: Math.max(3000, toInt(process.env.DUEL_WEAPON_PICK_MS, 5000)),
  ...
  prepTimeoutMs: Math.max(3000, toInt(process.env.DUEL_PREP_MS, 10000)),
